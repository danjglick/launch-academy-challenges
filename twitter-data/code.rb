...
twitter_data.each do |person|
  print "#{person.keys[0]}: "
  description = person.values[0]["description"]
  if description.nil?
    puts "NA"
  else
    puts description
...
puts ""
twitter_data.each do |person|
  puts "#{person.keys[0]} has #{person.values[0]["number of followers"]} followers."
end

# Each username and the length of their latest tweet.
puts ""
twitter_data.each do |person|
  puts "#{person.keys[0]} used #{person.values[0]["latest tweet"].size} characters in their latest tweet."
end

# Each username followed by the total character count used in that user's last twenty tweets.
puts ""
twitter_data.each do |person|
  print "#{person.keys[0]} used "
  sum = 0
  person.values[0]["last twenty tweets"].each do |tweet|
    sum += tweet.size
  end
  puts "#{sum} characters in their last twenty tweets."
end
...
winner = ""
twitter_data.each do |person|
  num_followers = person.values[0]["number of followers"]
  if num_followers > most_followers
    most_followers = num_followers
    winner = person.keys[0]
  end
end
puts "#{winner} has the most followers."
...
winner = ""
twitter_data.each do |person|
  num_friends = person.values[0]["number of friends"]
  if num_friends > most_friends
    most_friends = num_friends
    winner = person.keys[0]
  end
end
puts "#{winner} has the most friends."
...
winner = 0
twitter_data.each do |person|
  num_tweets = person.values[0]["number of tweets"]
  if num_tweets > most_tweets
    most_tweets = num_tweets
    winner = person.keys[0]
  end
end
puts "#{winner} has the most tweets."

# Which users have a description listed in twitter_data?
puts ""
puts "The following users have a listed description: "
twitter_data.each do |person|
  if !person.values[0]["description"].nil?
    puts person.keys[0]
  end
end
...
puts ""
puts "The following users have a listed location: "
twitter_data.each do |person|
  if !person.values[0]["location"].nil?
    puts person.keys[0]
  end
end
puts ""
